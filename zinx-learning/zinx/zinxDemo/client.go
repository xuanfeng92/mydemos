...
func main() {
	...
	for{
		...
		dp := znet.NewDataPack()
		binaryMsg, e := dp.Pack(znet.NewMsgPackage(0, []byte("Zinx ping client Test Message")))
		if e!=nil{
			fmt.Println("Client Pack error:",e)
			break
		}
		...
		if _, e = io.ReadFull(conn, binaryHead); e!=nil{
			fmt.Println("Client read head error:", e)
			break
		}
		// 2. 将二进制的head拆包到msg结构体中
		message, e := dp.Unpack(binaryHead)
		if e!=nil{
			fmt.Println("Client unpack head error:", e)
			break
		}
		...
		if message.GetMsgLen()>0{
			...
			fmt.Println("-->Client recv msg: ID=", msg.GetMsgId(), " len=", msg.GetMsgLen(), " data=", string(msg.GetData()))
		}
